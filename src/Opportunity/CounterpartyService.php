...
final class CounterpartyService
{
    ...
    public function history(int $id): array
    {
        return array_map(static function ($r): array {
            $v = (array) $r;
            // Counterparty roles remain nullable strings.
            ...
            return $v;
        }, $this->db->fetchAll('SELECT * FROM opportunity_counterparty WHERE opportunity_id = ? ORDER BY id DESC', $id));
    }

    public function save(int $id, int $expectedVersion, CounterpartyInput $input, ?int $actor, ?int $sourceVersion = null): void
    {
        $this->db->transaction(function () use ($id, $expectedVersion, $input, $actor, $sourceVersion): void {
            ...
            $this->db->query('INSERT INTO opportunity_counterparty', ['opportunity_id' => $id, 'source_version_id' => $sourceVersion, 'value' => $input->value, 'reason' => $input->reason, 'confidence' => $input->confidence, 'verified_at' => $input->verifiedAt, 'actor_user_id' => $actor, 'created_at' => new \DateTimeImmutable()]);
            $this->db->query('UPDATE opportunities SET lock_version = lock_version + 1, updated_at = ? WHERE id = ?', new \DateTimeImmutable(), $id);
            $this->audit->record('opportunity.counterparty_classified', $actor, ['opportunity_id' => $id, 'source_version_id' => $sourceVersion, 'value' => $input->value]);
        });
    }
}
