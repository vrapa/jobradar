...
For each page record ordinal, public URL, number displayed, details opened and next-page/end indicator. Increment observed counts only once per actual unit. Use canonical URLs/import deduplication across recovered work. Report a lower bound/unknown when dynamic pagination prevents a reliable total; never fabricate counts to satisfy completion validation.

Synthetic pilot: `http://jobradar.localhost/execution-probe.html`, query “synthetic PHP”, limit 1. Verify marker jobradar-execution-probe-v1, heading Ukázkový PHP projekt and explicit end count 1. No real job or application. This proves Chrome reading only, not a real portal adapter or scheduled execution.

Public and login-gated production pilots require selected actual source IDs, configured filters and user-confirmed current request. A login gate is a useful negative test, not authorization to log in or bypass it.
